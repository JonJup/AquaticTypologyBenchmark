...
param_combinations <- tibble(
  centroid_adjustment = runif(n = n, min = -1, max = 1), 
  compactness = runif(n = n, min = -1, max = 1)
)
q.newenv <- pmap(param_combinations, function(centroid_adjustment, compactness) {
  adjust_clusters(
...
j.contraction.points %<>% append(param_combinations$compactness)
j.contraction.centroids %<>% append(param_combinations$centroid_adjustment)